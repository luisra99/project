...
import { formatDisplayDate, formatDisplayTime, getStatusColor, getStatusIcon } from '@/lib/utils';
import { CheckCircle, Clock, Loader2, User, Users, UtensilsCrossed, XCircle } from 'lucide-react';

interface AdminReservationListProps {
  status: string;
}

export default function AdminReservationList({ status }: AdminReservationListProps) {
  const { reservations, loading, updateStatus, refreshReservations } = useReservations();
  ...
  const handleStatusUpdate = async (id: string, newStatus: Reservation['status']) => {
    setProcessing(id);
