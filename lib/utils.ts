...
import { getReservationsByDate, isTimeSlotAvailable } from './db';
...
export const getAvailabilityForDate = (dateString: string): DayAvailability => {
  const reservations = getReservationsByDate(dateString);
  
  const timeSlots: TimeSlot[] = AVAILABLE_TIMES.map(time => ({
    id: `${dateString}-${time}`,
    time,
    available: isTimeSlotAvailable(dateString, time)
  }));
